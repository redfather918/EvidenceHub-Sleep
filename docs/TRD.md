...
### 9.5 Article 模块（Module 5：Claim → Article，V3 新增）

把任意 Claim 自动渲染为循证长文（Deep Dive）。内容由 Claim 的结构化字段自动拼装，无需人工撰写。

#### 9.5.1 `composeArticle`（`src/lib/article.ts`）

`composeArticle(claim: ClaimWithRelations): ArticleContent` —— 将 Claim 转换为文章结构：

- `title` = `claim.text`；`kicker` = `category · topic.name`；`subtitle` = `summary`
- `lede`：一段自动导语，综合 studyCount / rctCount / metaCount / evidenceScore / confidence
- `sections[]`：`The Claim` / `What the Research Shows`（研究亮点 bullets）/ `How It Works`（mechanism）/ `Who Might Benefit`（populationFits）/ `Recommended Dose`（doseMappings）/ `Limitations & Caveats`
- `faq`：原样透传 `claim.faq`（页面用 `<details>` 渲染）
- `readingMinutes`：按语料词数 / 200 估算
- `relatedClaims` / `studies`：透传用于文末区块

#### 9.5.2 文章页（`src/app/article/[slug]/page.tsx`）

服务端组件，ISR 1h。结构：Hero（Kicker / H1 / 副标题 / 元信息）→ 证据评分条 → Lede → Sections → FAQ → References（PMID/DOI 外链）→ Related Claims → Disclaimer。

SEO：`generateMetadata`（canonical `/article/[slug]`、OG/Twitter）+ JSON-LD：`Article`（`generateArticleJsonLd`）+ `FAQPage` + `BreadcrumbList`（`src/lib/seo.ts`）。

#### 9.5.3 集成点

- Claim 详情页 header：「📖 Read as article →」链接到 `/article/[slug]`
- `ClaimCard`（`src/components/ClaimCard.tsx`）：重构为 `div` 容器 + 底部「📖 Article」链接（避免嵌套 `<a>`）
- 首页 Stats 栏下方：「Featured Deep Dive」渐变横幅，优先展示 sleep duration 主题 Claim（`/article/[slug]`）
