...
export function ClaimCard({ claim }: { claim: Claim }) {
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md hover:border-brand-300 transition-all">
      <Link href={`/claim/${claim.slug}`} className="block flex-1">
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
        <span className="text-xs text-gray-400">{claim.evidenceScore}/100 confidence</span>
        <Link
          href={`/article/${claim.slug}`}
